...
int main(int argc, char** argv){
    ...
    float *first_layer_bias_ds = new float[NUM_NEURONS]();
    float *second_layer_bias =  new float[(int)NUM_NEURONS/EPOCH_SIZE]();
    float *second_layer_bias_ds = new float[(int)NUM_NEURONS/EPOCH_SIZE]();
        ...
        for(int j = 0; j < EPOCH_SIZE; j++){
            ...
            for(int i = 0; i < BATCH_SIZE; i++){
                ...
                for(int k = 0; k < (int) NUM_NEURONS/EPOCH_SIZE; k++){
                    ...
                    second_layer_bias_ds[k] = soft_max_layer_ds[k] / BATCH_SIZE;
                }
                ...
                for(int n = 0; n < NUM_NEURONS; n++){
                    ...
                    first_layer_bias_ds[n] = second_layer_ds[n] / BATCH_SIZE;
                }
                ...
                for(int k = 0; k < (int) NUM_NEURONS/EPOCH_SIZE; k++){
                    for(int n = 0; n < NUM_NEURONS; n++){
                        fully_connected_layer_w[k][n] -= fully_connected_layer_ds[k][n] * learning_rate;
                        fully_connected_layer_ds[k][n] = 0;
                    }
                    second_layer_bias[k] -= second_layer_bias_ds[k] * learning_rate;
                    second_layer_bias_ds[k] = 0;
                }

                for(int n = 0; n < NUM_NEURONS; n++){
                    for(int r = 0; r < ROWS; r++){
                        for(int c = 0; c < COLS; c++){
                            input_layer_w[n][r][c] -=  input_layer_ds[n][r][c] * learning_rate;
                            //std::cout << input_layer_ds[n][r][c] << std::endl;
                            //std::cout << input_layer_w[n][r][c] << std::endl;
                            input_layer_ds[n][r][c] = 0;
                        }
                    }
                    first_layer_bias[n] -= first_layer_bias_ds[n] * learning_rate;
                    first_layer_bias_ds[n] = 0;
                }
            }
            if(j % 100 == 0){
                //std::cout << input_layer_w[0][0][0] << std::endl;
